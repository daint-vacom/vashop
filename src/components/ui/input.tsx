...
function Input({
  ...
  const buildEmptyValue = () => {
    ...
    if (emptyAs === 'undefined') return undefined;
    return '';
  };
  ...
  if (!mask) {
    return <input data-slot="input" type={type} {...commonProps} />;
  }

  if (mask === 'number') {
    const { onChange: _ignoreOnChange, ...restCommonProps } = commonProps;
    ...
    return (
      <NumericFormat
        ...
        onValueChange={(values: any) => {
          ...
          if (raw === '') onChange?.(buildEmptyValue());
          else onChange?.(raw);
        }}
      />
    );
